...
function get_data(limit,page,filter){
    fetch(`http://127.0.0.1:3000/table_data/${limit}/${page}/${filter}`).
    then(res => {
        if (!res.ok) {
...
function deleting(event){
    ...
    d.remove();
    fetch(`http://127.0.0.1:3000/deleting/${g}`,{
        method:'DELETE'
    })
    .then(res => {
        if (!res.ok) {
...
function editing(event){
    ...
    fetch(`http://127.0.0.1:3000/updating2${t}`,{
        method:'PUT',
        body:forms
    })
    .then(res => {
...
function submittings(e){
    ...
    //     return;
    // }
    fetch('http://127.0.0.1:3000/uploading2',{
        method:'POST',
        body:forms
    })
    .then(res => {
...
function searching(event){    
    ...
    if (searched.length > 2) {
        fetch(`http://127.0.0.1:3000/searching/${event.target.value}`).
    then(res => {
        if (!res.ok) {
            return res.text().then(response => {
                throw new Error(response.substring(11,response.length-2))
...
function viewing(event){
    ...
    fetch(`http://127.0.0.1:3000/gettingImage/${i}`).
    then(res => {
        if (!res.ok) {
            return res.text().then(response => {
                throw new Error(response.substring(11,response.length-2))
